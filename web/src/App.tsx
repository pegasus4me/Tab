...
function sharedCode() { return new URLSearchParams(window.location.hash.slice(1)).get("activity"); }
...
export default function App({ apiUrl }: Props) {
  ...
  if (route === "activity") return <ActivityPage activity={activity} loading={loading} error={error} loggedIn={loggedIn} />;
...
function CreateActivity({ apiUrl, onCreated }: { apiUrl: string; onCreated: (activity: Activity) => void }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); setBusy(true); setError(""); try { const data = await request(apiUrl, "/activities", { ...form, contribution: form.contribution.replace(",", "."), startsAt: new Date(form.startsAt).toISOString(), fundingDeadline: new Date(form.fundingDeadline).toISOString() }, true); onCreated(data.activity); } catch (reason) { setError(reason instanceof Error ? reason.message : "Impossible de créer l’activité."); } finally { setBusy(false); } }
...
function ActivityPage({ activity, loading, error, loggedIn }: { activity: Activity | null; loading: boolean; error: string; loggedIn: boolean }) {
  const shareUrl = window.location.href; const text = activity ? `${activity.title} — ${date.format(new Date(activity.startsAt))}, ${money.format(Number(activity.contribution))} par personne. Tu viens ?` : "";
